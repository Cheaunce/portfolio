...
app.layout = html.Div([
    ...
    html.H1("Automobile Sales Statistics Dashboard", style={'textAlign': 'left', 'color': '#503D36', 'fontSize': '24px'}),
    ...
    html.Div(id='output-container', className='output-text', style={'textAlign': 'left', 'color':'#503D36', 'fontSize':'24px'}),])
])
...
@app.callback(
    ...
    [Input(component_id='select_stat', component_property='value'), Input(component_id='select-year', component_property='value')])

def update_output_container(select_stat, select_year):
    if select_stat == 'Recession Period Statistics':
    ...
    elif select_stat == 'Yearly Statistics' and select_year:
        ...
        Y_chart4 = dcc.Graph(
            figure=px.pie(
                ...
                values='Advertising_Expenditure',
                names='Vehicle_Type',
                title='Total Advertisement Expenditure for Each Vehicle Type in {}'.format(select_year)
            )
        )
